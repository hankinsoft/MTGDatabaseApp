...
- (void)addToScrollView:(UIScrollView *)scrollView {
    ...
    // NOTE: Some classes such as HSExpandableTableView wrap their delegate. In this case
    // we hit an infinate loop. We will check and see if a rawDelegate is provided. If so,
    // use that instead.
    if([self.scrollView respondsToSelector: @selector(rawDelegate)])
    {
        self.originalDelegate = [self.scrollView performSelector: @selector(rawDelegate)];
    }
    else
    {
        self.originalDelegate = self.scrollView.delegate;
